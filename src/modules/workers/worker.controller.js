...
const pickLocationLabel = (record) => {
  ...
  if (record.job && record.job.business && typeof record.job.business === 'object' && record.job.business.name) {
    return record.job.business.name;
  }
  return null;
};
...
const mapRecordToScheduleEntry = (record, context) => {
  ...
  const { now, startOfToday, endOfToday } = context;
  ...
  const endReference = scheduledEnd || scheduledStart;
  const isPast = endReference < startOfToday;
  const isFuture = scheduledStart > endOfToday;
  const isToday = !isPast && !isFuture;
  const isUpcoming = !isPast;
  const isInProgress = !isPast && !isFuture && (
    record.status === 'clocked-in' ||
    (scheduledStart <= now && endReference >= now)
  );
...
exports.getWorkerAttendanceSchedule = catchAsync(async (req, res, next) => {
  ...
  if (req.user.userType === 'worker' && req.user._id.toString() !== workerId.toString()) {
    return next(new AppError('You can only view your own attendance schedule', 403));
  }

  const { status, from, to, jobId, businessId } = req.query;
  const filter = { worker: workerId };
  ...
  if (from) {
    ...
    if (Number.isNaN(fromDate.valueOf())) {
      return next(new AppError('Invalid from date parameter', 400));
    }
  ...
  if (to) {
    ...
    if (Number.isNaN(toDate.valueOf())) {
      return next(new AppError('Invalid to date parameter', 400));
    }
  ...
  const records = await AttendanceRecord.find(filter)
    .populate([
      {
        path: 'job',
        select: 'title hourlyRate location business',
  ...
  records.forEach(record => {
    const entry = mapRecordToScheduleEntry(record, { now, startOfToday, endOfToday });
    if (!entry) {
      return;
    }
    ...
    grouped.get(entry.date).entries.push(entry);
  });

  const schedule = Array.from(grouped.values())
    .map(section => {
      section.entries.sort((a, b) => new Date(a.scheduledStart) - new Date(b.scheduledStart));
      const pastDay = section.entries.every(entry => entry.isPast);
      const futureDay = section.entries.every(entry => entry.isFuture);
      const todayDay = section.entries.some(entry => entry.isToday);
      const hasInProgress = section.entries.some(entry => entry.isInProgress);
      return {
        ...section,
        isPast: pastDay,
        isUpcoming: !pastDay,
        ...
        isToday: todayDay,
        hasInProgress,
        totals: buildSectionTotals(section.entries)
      };
    })
    .sort((a, b) => new Date(a.date) - new Date(b.date));

  const pastSections = schedule.filter(section => section.isPast);
  const firstUpcomingSection = schedule.find(section => !section.isPast) || null;
  ...
  const response = {
    ...
    summary: {
      ...
      firstUpcomingDate: firstUpcomingSection ? firstUpcomingSection.date : null,
      lastPastDate: pastSections.length > 0 ? pastSections[pastSections.length - 1].date : null,
      range: {
        start: schedule.length > 0 ? schedule[0].date : null,
        end: schedule.length > 0 ? schedule[schedule.length - 1].date : null
      }
    },
    schedule
  };
