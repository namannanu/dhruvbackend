...
// Get a single job by ID with proper business location population
exports.getJob = catchAsync(async (req, res, next) => {
  const { jobId } = req.params;

  // Define populate options based on user type
  const businessPopulateForDetail =
    req.user.userType === 'employer'
      ? {
          path: 'business',
          select: 'businessName name logoUrl logo owner location address businessAddress formattedAddress',
          populate: { path: 'owner', select: 'email firstName lastName' },
        }
      : {
          path: 'business',
          select: 'businessName name logoUrl logo location address businessAddress formattedAddress', // ✅ include location for workers too
        };
  ...
  // For employers, add additional population
  if (req.user.userType === 'employer') {
  ...
  // Build the response using the same buildJobResponse function
  const responseData = await buildJobResponse(job, req.user);

  // Belt-and-suspenders fallback for businessAddress
  if (!responseData.businessAddress) {
    const loc =
      responseData.location ||
      responseData.business?.location ||
      responseData.businessDetails?.location;

    if (loc) {
      const label =
        loc.formattedAddress ||
        [loc.line1, loc.city, loc.state, loc.postalCode, loc.country]
          .filter(Boolean)
          .join(', ');
      if (label) responseData.businessAddress = label;
    }
  }
